...
void TimersMngr_Init(void)
{
    ...
    while (i--)
    {
    	sTimersObjArray[i].u32TimerCount = TIMER_INACTIVE;	/* Set all timers inactive */
...
void TimersMngr_PeriodicTask(void)
{
    ...
    if(time > 0u)  /* Update the timers if needed */
    {
        for(i=0; i<su8Timers; i++) /* loop through all created timers */
        {
            ...
            if(sTimersObjArray[i].u32TimerCount != TIMER_INACTIVE)	
            {
            	/*Updates count of the timer*/
                if(sTimersObjArray[i].u32TimerCount > time )
                {
                	sTimersObjArray[i].u32TimerCount -= time;
                }
                else { 
                	sTimersObjArray[i].u32TimerCount = TIMER_INACTIVE;
                }
                
                /* Call callback if timer has elapsed and if there is a registered callback for the expired timer */
                if(TIMER_INACTIVE == sTimersObjArray[i].u32TimerCount) 
                {
...
/* --------------------------------------------------------------------------------------------- */
void TimersMngr_StartTimer(u8 u8TimerId, u32 u32Microseconds)
{
	
	if (u8TimerId <= su8Timers && sTimersObjArray[u8TimerId].u32TimerCount == TIMER_INACTIVE) /* Check the timerID is valid */
	{
		sTimersObjArray[u8TimerId].u32TimerCount = u32Microseconds;
		su8ActiveTimers++;
	}
}
...
void TimersMngr_StopTimer(u8 u8TimerId)
{
	...
	{
		su8ActiveTimers--;
		sTimersObjArray[u8TimerId].u32TimerCount = TIMER_INACTIVE;
	}
}
...
{
    return (TIMER_INACTIVE == sTimersObjArray[u8TimerId].u32TimerCount);
}
